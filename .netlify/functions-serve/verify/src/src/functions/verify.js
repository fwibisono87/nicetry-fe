...
// src/functions/impl/auth/verify.ts
var verify = async (token) => {
  ...
  try {
    const response = await fetch("https://nicetry.franciswibisono.com/user/verify", requestOptions);
...
var handler = async (event) => {
  ...
  try {
    if (token) {
      ...
      return {
        statusCode: 400,
        body: JSON.stringify({ message: "Token is null." })
      };
    }
    return {
      statusCode: 200,
      body: JSON.stringify(__spreadValues({}, result))
    };
  } catch (e2) {
    return {
      statusCode: 500,
      body: JSON.stringify({ message: "Something went wrong", error: e2 })
    };
  }
};
