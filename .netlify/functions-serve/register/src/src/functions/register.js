...
// src/functions/impl/auth/register.ts
var register = async (email, password, firstName, lastName, imageUrl, role) => {
  const body = JSON.stringify({ email, password, firstName, lastName, imageUrl, role });
  const requestOptions = {
    method: "POST",
  ...
  try {
    const response = await fetch("https://nicetry.franciswibisono.com/user/user/create", requestOptions);
...
var handler = async (event) => {
  ...
  const firstName = body == null ? void 0 : body.firstName;
  const lastName = body == null ? void 0 : body.lastName;
  const imageUrl = body == null ? void 0 : body.imageUrl;
  const role = body == null ? void 0 : body.role;
  let result;
  try {
    if (email && password && firstName && lastName && imageUrl && role) {
      result = await register(email, password, firstName, lastName, imageUrl, role);
    } else {
      return {
        statusCode: 400,
        body: JSON.stringify({ message: "Email or password is null." })
      };
    }
    return {
      statusCode: 200,
    ...
    if (e2 instanceof Error) {
      if (e2.message.includes("409")) {
        return {
          statusCode: 409,
          body: JSON.stringify({
            message: "Conflict: User with the same email already exists",
            error: e2.message
          })
        };
      } else {
        return {
          statusCode: 500,
          body: JSON.stringify({ message: "Something went wrong", error: e2.message })
        };
      }
    } else {
      return {
        statusCode: 500,
        body: JSON.stringify({ message: "Something went wrong", error: "Unknown error" })
      };
    }
  }
};
